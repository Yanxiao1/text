...
#调用
main_bg('./resources/background1.jpg')
...
with cols[2]:
    ...
    if gen:
        step=50
        generateFunction(image_path,prompt1=prompt_1,prompt2=prompt_2, model=ldm_stable ,num_steps=step,encrypto_flag=True)
        # 在这里调用生成图片的函数，例如 generate_image(prompt1, prompt2)
        # 生成的图片保存在 generated_image_path 变量中
        file_name_without_extension = os.path.splitext(uploaded_image_filename)[0]
