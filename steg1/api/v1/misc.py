...
def hide_and_recover_image(image: np.ndarray, prompt1: str, prompt2: str, model, num_steps: int = 50) -> Tuple[np.ndarray, np.ndarray]:
    # 初始化ODESolve类
    ode = ODESolve(model, num_steps)
    
    # 将输入图像转换为潜在空间表示
    image_latent = ode.image2latent(image)
    
    # 隐藏图像
    latent_noise = ode.invert(prompt1, image_latent, is_forward=True)
    image_hide_latent = ode.invert(prompt2, latent_noise, is_forward=False)
    image_hide = ode.latent2image(image_hide_latent)
    
    # 恢复图像
    image_hide_latent_reveal = ode.image2latent(image_hide)
    latent_noise = ode.invert(prompt2, image_hide_latent_reveal, is_forward=True)
    image_reverse_latent = ode.invert(prompt1, latent_noise, is_forward=False)
    image_reverse = ode.latent2image(image_reverse_latent)
    
    return image_hide, image_reverse
def generateFunction(img_path: str, prompt1: str, prompt2: str, model, num_steps,encrypto_flag: bool):
    try:
        img_name = os.path.splitext(os.path.basename(img_path))[0]  # Extract filename without extension
       ...
       #hidden
        if encrypto_flag==True:
            latent_noise = ode.invert(prompt1, image_latent, is_forward=True)
            image_hide_latent = ode.invert(prompt2, latent_noise, is_forward=False)
            image_hide = ode.latent2image(image_hide_latent)
            cv2.imwrite(os.path.join(output_dir, f"{img_name}_hidden.jpg"), cv2.cvtColor(image_hide, cv2.COLOR_RGB2BGR))
        
        #reverse
        elif encrypto_flag== False:
            image_hide_latent_reveal = ode.image2latent(image_hide)
            latent_noise = ode.invert(prompt2, image_hide_latent_reveal, is_forward=True)
            image_reverse_latent = ode.invert(prompt1, latent_noise, is_forward=False)
            image_reverse = ode.latent2image(image_reverse_latent)
            cv2.imwrite(os.path.join(output_dir, f"{img_name}_reverse.jpg"), cv2.cvtColor(image_reverse, cv2.COLOR_RGB2BGR))
...
if __name__ == "__main__":
    # Assuming ldm_stable is defined somewhere
    generateFunction("./uploaded_images/cat.png", prompt1="a cat", prompt2="a dog", model=ldm_stable, num_steps=5,encrypto_flag=True)
